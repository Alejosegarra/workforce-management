...
if (isSupabaseConfigured) {
    ...
    console.error(
        ...
        "***\n" +
        "*** 1. Para desarrollo local: Abre 'services/supabase.ts' y reemplaza los placeholders.\n" +
        "*** 2. Para despliegue (Vercel): Configura las variables de entorno\n" +
        "***    VITE_SUPABASE_URL y VITE_SUPABASE_ANON_KEY en tu proyecto.\n" +
        "********************************************************************************"
    );
    // Provide a mock object that prevents the app from crashing on startup.
    // The main App component will see `isSupabaseConfigured` is false and show a setup screen.
    supabase = {
        auth: {
            getSession: () => new Promise(res => res({ data: { session: null }, error: null })),
